...
function Show-CommandResult {
    ...
    $outputText = if ($null -eq $Result.Output) { "" } else { ($Result.Output | Out-String).Trim() }
...
function Ensure-DockerfileExists {
    ...
    if (-not (Test-Path $Path)) {
        throw "Dockerfile not found at path: $Path"
    }
...
function Ensure-ContainerAppsExtension {
    ...
    $result = Invoke-AzureCli extension list `
    ...
    if ((($result.Output | Out-String).Trim()) -eq "containerapp") {
        Write-Host "Azure Container Apps extension is already installed." -ForegroundColor Green
        Pause-ForInspection
        return
    }

    [void](Run-Step -Title "Installing Azure Container Apps extension" -PauseAfter {
...
function Ensure-ResourceProviderRegistered {
    ...
    $registerResult = Invoke-AzureCli provider register --namespace $Namespace
...
function Ensure-PrimaryResourceGroup {
    ...
    [void](Run-Step -Title "Creating Azure resource group" -PauseAfter {
...
function Ensure-ContainerRegistry {
    ...
    [void](Run-Step -Title "Creating Azure Container Registry" -PauseAfter {
...
function Ensure-ContainerAppsEnvironment {
    $existingEnvironmentId = Get-ExistingResourceValue -Arguments @(
    ...
    if ($existingEnvironmentId) {
        ...
        Write-Host "Resource ID: $existingEnvironmentId"
        Pause-ForInspection
        return
    }
...
function Get-ContainerRegistryCredentials {
    $usernameResult = Invoke-AzureCli acr credential show `
        --name $ContainerRegistryName `
        --query username `
        --output tsv

    if ($usernameResult.ExitCode -ne 0) {
        Show-CommandResult -Result $usernameResult
        throw "Could not retrieve Azure Container Registry username."
    }

    $passwordResult = Invoke-AzureCli acr credential show `
        --name $ContainerRegistryName `
        --query "passwords[0].value" `
        --output tsv

    if ($passwordResult.ExitCode -ne 0) {
        Show-CommandResult -Result $passwordResult
        throw "Could not retrieve Azure Container Registry password."
    }

    return @{
        Username = ($usernameResult.Output | Out-String).Trim()
        Password = ($passwordResult.Output | Out-String).Trim()
    }
}

function Ensure-ContainerAppsJob {
    ...
    $registryCredentials = Get-ContainerRegistryCredentials
    ...
    [void](Run-StreamingStep `
        ...
        -PauseAfter {
            Invoke-AzureCli -StreamOutput containerapp job create `
                ...
                --registry-username $registryCredentials.Username `
                --registry-password $registryCredentials.Password `
                --env-vars `
...
function Ensure-ManagedIdentity {
    [void](Run-Step -Title "Assigning system-managed identity to Azure Container Apps Job" -PauseAfter {
    ...
    [void](Run-Step -Title "Retrieving Azure Container Apps Job principal ID" -PauseAfter {
...
function Ensure-AcrPullRoleAssignment {
    $principalId = Get-ContainerAppsJobPrincipalId

    $registryResult = Invoke-AzureCli acr show `
        --name $ContainerRegistryName `
        --resource-group $PrimaryResourceGroupName `
        --query id `
        --output tsv

    if ($registryResult.ExitCode -ne 0) {
        Show-CommandResult -Result $registryResult
        throw "Could not retrieve the resource ID for registry '$ContainerRegistryName'."
    }

    $registryResourceId = ($registryResult.Output | Out-String).Trim()
...
function Ensure-BlobRoleAssignments {
    $principalId = Get-ContainerAppsJobPrincipalId

    foreach ($storageAccount in $StorageAccounts) {
        $storageResult = Invoke-AzureCli storage account show `
        ...
        if ($storageResult.ExitCode -ne 0) {
            Show-CommandResult -Result $storageResult
            throw "Could not retrieve the resource ID for storage account '$($storageAccount.Name)'."
        }

        $storageResourceId = ($storageResult.Output | Out-String).Trim()
...
function Ensure-RegistryConfiguration {
    [void](Run-Step -Title "Configuring Azure Container Apps Job to use system-managed identity for Azure Container Registry" -PauseAfter {
...
[void](Run-Step -Title "Setting Azure subscription" -PauseAfter {
...
Ensure-ContainerAppsEnvironment
Ensure-ContainerAppsJob
Ensure-ManagedIdentity
Ensure-AcrPullRoleAssignment
Ensure-BlobRoleAssignments
Ensure-RegistryConfiguration
